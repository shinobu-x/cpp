...
// Test std array buffers async read
void test_26() {
  boost::asio::io_service ios;
  stream s(ios);
  char read_buf[sizeof(read_data)];
...
// Test vector buffers async read
void test_27() {
  boost::asio::io_service ios;
  stream s(ios);
  char read_buf[sizeof(read_data)];
...
void test_28() {
  ...
  s.reset(read_data, sizeof(read_data));
  s.next_read_length(20);
  sb.consume(sb.size());
  called = false;
  boost::asio::async_read(s, sb, boost::asio::transfer_at_least(10),
    boost::bind(async_read_handler, _1, _2, 20, &called));
  ios.reset();
  ios.run();
  assert(called);
  assert(sb.size() == 20);
  assert(s.check_buffers(sb.data(), 20));
...
auto main() -> decltype(0) {
  ...
  test_24(); test_25(); test_26(); test_27(); test_28();
