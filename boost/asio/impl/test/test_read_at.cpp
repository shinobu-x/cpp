...
void test_12() {
  ...
  ec = boost::system::error_code();
  bytes_transferred = boost::asio::read_at(s, 0, sb,
    boost::asio::transfer_at_least(45), ec);
  assert(!ec);
  assert(bytes_transferred == sizeof(read_data));
  assert(s.check_buffers(0, sb.data(), sizeof(read_data)));
  ...
  assert(s.check_buffers(0, sb.data(), 45));
  ...
  assert(!ec);
  assert(bytes_transferred == 45);
  assert(s.check_buffers(1234, sb.data(), 45));
...
void test_13() {
  ...
  s.reset(read_data, sizeof(read_data));
  s.next_read_length(45);
  memset(read_buf, 0, sizeof(read_buf));
  called = false;
  boost::asio::async_read_at(s, 0, buffers,
    boost::asio::transfer_at_least(10),
    boost::bind(async_read_handler, _1, _2, 45, &called));
  ios.reset();
  ios.run();
  assert(called);
  assert(s.check_buffers(0, buffers, 45));
  ...
  ios.run();
  assert(called);
  assert(s.check_buffers(1234, buffers, 45));
  ...
  called = false;
  boost::asio::async_read_at(s, 0, buffers,
    boost::asio::transfer_at_least(45),
...
// Test boost array buffers async read at
void test_14() {
  boost::asio::io_service ios;
  stream_access_device s(ios);
  char read_buf[sizeof(read_data)];
  ...
  memset(read_buf, 0, sizeof(read_buf));
  called = false;
  boost::asio::async_read_at(s, 1234, buffers, boost::asio::transfer_all(),
    boost::bind(async_read_handler, _1, _2, sizeof(read_data), &called));
  ios.reset();
  ios.run();
  assert(called);
