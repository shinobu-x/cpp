...
void test_1() {
  try {
    ...
    int l6 = stream1.async_fill(lazy);
    (void)l6;
    ...
    int l7 = stream1.async_read_some(
      boost::asio::buffer(mutable_char_buffer), lazy);
    (void)l7;
    int l8 = stream1.async_read_some(mutable_buffers, lazy);
    (void)l8;
    int l9 = stream1.async_read_some(boost::asio::null_buffers(), lazy);
    (void)l9;
  } catch (...) {}
}
} // namespace
...
void test_1() {
  ...
  const char write_data[] =
    "ABCDEFGHIJKLMNOPQRLSTUVWXYZabcdefghijklmnopqrstuvwxyz";
  boost::asio::const_buffer write_buf = boost::asio::buffer(write_data);
