...
MODULE_DESCRIPTION("Block Device Test Part3");
...
static int sector_s = 512;
static int sector_n = 102400;
...
static void do_transfer(struct bdev_t *dev, sector_t sector,
  ...
  unsigned long offset = sector * sector_s; /* Object start byte */
  unsigned long nbytes = nsect * sector_s;  /* Bytes from offset */
...
int setgeo(struct block_device* block_device, struct hd_geometry* geo) {
  long size;
  size = Bdev.size * (sector_s / KERNEL_SECTOR_SIZE);
...
static int __init bdev_init(void) {
  ...
  queue = blk_init_queue(do_request,  &Bdev.lock);

  if (queue == NULL)
    goto out;

  blk_queue_logical_block_size(queue, sector_s);
